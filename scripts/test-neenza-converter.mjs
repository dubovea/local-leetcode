...
assert(problem.functionName === "groupAnagrams", "function name should be extracted from JS snippet");
assert(problem.testCases[0].input === 'strs = ["eat","tea","tan","ate","nat","bat"]', "input should be parsed from example");
assert(problem.testCases[0].expected === '[["bat"],["nat","tan"],["ate","eat","tea"]]', "expected output should be parsed from example");
assert(problem.judgeMode === "unordered-array", "any-order tasks should use unordered-array judge internally");
